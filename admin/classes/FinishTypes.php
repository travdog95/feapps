...
/**
 * Table class for FinishTypes
 */
class FinishTypes extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'FinishTypes';
		$this->TableName = 'FinishTypes';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "[dbo].[FinishTypes]";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// FinishType_Idn
		$this->FinishType_Idn = new DbField('FinishTypes', 'FinishTypes', 'x_FinishType_Idn', 'FinishType_Idn', '[FinishType_Idn]', 'CAST([FinishType_Idn] AS NVARCHAR)', 3, 4, -1, FALSE, '[FinishType_Idn]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->FinishType_Idn->IsAutoIncrement = TRUE; // Autoincrement field
		$this->FinishType_Idn->IsPrimaryKey = TRUE; // Primary key field
		$this->FinishType_Idn->Nullable = FALSE; // NOT NULL field
		...
		// Name
		$this->Name = new DbField('FinishTypes', 'FinishTypes', 'x_Name', 'Name', '[Name]', '[Name]', 200, 100, -1, FALSE, '[Name]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Name->Sortable = TRUE; // Allow sort
		$this->fields['Name'] = &$this->Name;

		// ShortName
		$this->ShortName = new DbField('FinishTypes', 'FinishTypes', 'x_ShortName', 'ShortName', '[ShortName]', '[ShortName]', 200, 30, -1, FALSE, '[ShortName]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ShortName->Sortable = TRUE; // Allow sort
		$this->fields['ShortName'] = &$this->ShortName;

		// Rank
		$this->Rank = new DbField('FinishTypes', 'FinishTypes', 'x_Rank', 'Rank', '[Rank]', 'CAST([Rank] AS NVARCHAR)', 3, 4, -1, FALSE, '[Rank]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ActiveFlag
		$this->ActiveFlag = new DbField('FinishTypes', 'FinishTypes', 'x_ActiveFlag', 'ActiveFlag', '[ActiveFlag]', '[ActiveFlag]', 11, 2, -1, FALSE, '[ActiveFlag]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->ActiveFlag->Sortable = TRUE; // Allow sort
		$this->ActiveFlag->DataType = DATATYPE_BOOLEAN;
		$this->ActiveFlag->Lookup = new Lookup('ActiveFlag', 'FinishTypes', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[FinishTypes]";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->FinishType_Idn->setDbValue($conn->insert_ID());
			$rs['FinishType_Idn'] = $this->FinishType_Idn->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('FinishType_Idn', $rs))
				AddFilter($where, QuotedName('FinishType_Idn', $this->Dbid) . '=' . QuotedValue($rs['FinishType_Idn'], $this->FinishType_Idn->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	{
		return "[FinishType_Idn] = @FinishType_Idn@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('FinishType_Idn', $row) ? $row['FinishType_Idn'] : NULL;
		else
		...
		if ($val == NULL)
			...
			$keyFilter = str_replace("@FinishType_Idn@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	{
		global $Language;
		if ($pageName == "FinishTypesview.php")
			return $Language->phrase("View");
		elseif ($pageName == "FinishTypesedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "FinishTypesadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("FinishTypesview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("FinishTypesview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "FinishTypesadd.php?" . $this->getUrlParm($parm);
		else
			$url = "FinishTypesadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("FinishTypesedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("FinishTypesadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("FinishTypesdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "FinishType_Idn:" . JsonEncode($this->FinishType_Idn->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->FinishType_Idn->CurrentValue != NULL) {
			$url .= "FinishType_Idn=" . urlencode($this->FinishType_Idn->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("FinishType_Idn") !== NULL)
				$arKeys[] = Param("FinishType_Idn");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->FinishType_Idn->CurrentValue = $key;
			else
				$this->FinishType_Idn->OldValue = $key;
