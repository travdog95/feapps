...
/**
 * Table class for SystemTypes
 */
class SystemTypes extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'SystemTypes';
		$this->TableName = 'SystemTypes';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "[dbo].[SystemTypes]";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// SystemType_Idn
		$this->SystemType_Idn = new DbField('SystemTypes', 'SystemTypes', 'x_SystemType_Idn', 'SystemType_Idn', '[SystemType_Idn]', 'CAST([SystemType_Idn] AS NVARCHAR)', 3, 4, -1, FALSE, '[SystemType_Idn]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->SystemType_Idn->IsAutoIncrement = TRUE; // Autoincrement field
		$this->SystemType_Idn->IsPrimaryKey = TRUE; // Primary key field
		$this->SystemType_Idn->IsForeignKey = TRUE; // Foreign key field
		$this->SystemType_Idn->Nullable = FALSE; // NOT NULL field
		...
		// Name
		$this->Name = new DbField('SystemTypes', 'SystemTypes', 'x_Name', 'Name', '[Name]', '[Name]', 200, 100, -1, FALSE, '[Name]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Name->Sortable = TRUE; // Allow sort
		$this->fields['Name'] = &$this->Name;

		// Rank
		$this->Rank = new DbField('SystemTypes', 'SystemTypes', 'x_Rank', 'Rank', '[Rank]', 'CAST([Rank] AS NVARCHAR)', 3, 4, -1, FALSE, '[Rank]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Department_Idn
		$this->Department_Idn = new DbField('SystemTypes', 'SystemTypes', 'x_Department_Idn', 'Department_Idn', '[Department_Idn]', 'CAST([Department_Idn] AS NVARCHAR)', 3, 4, -1, FALSE, '[Department_Idn]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// ActiveFlag
		$this->ActiveFlag = new DbField('SystemTypes', 'SystemTypes', 'x_ActiveFlag', 'ActiveFlag', '[ActiveFlag]', '[ActiveFlag]', 11, 2, -1, FALSE, '[ActiveFlag]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->ActiveFlag->Sortable = TRUE; // Allow sort
		$this->ActiveFlag->DataType = DATATYPE_BOOLEAN;
		$this->ActiveFlag->Lookup = new Lookup('ActiveFlag', 'SystemTypes', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "SystemSubTypes") {
			$detailUrl = $GLOBALS["SystemSubTypes"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_SystemType_Idn=" . urlencode($this->SystemType_Idn->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "SystemTypeslist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[SystemTypes]";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->SystemType_Idn->setDbValue($conn->insert_ID());
			$rs['SystemType_Idn'] = $this->SystemType_Idn->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('SystemType_Idn', $rs))
				AddFilter($where, QuotedName('SystemType_Idn', $this->Dbid) . '=' . QuotedValue($rs['SystemType_Idn'], $this->SystemType_Idn->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('SystemType_Idn', $row) ? $row['SystemType_Idn'] : NULL;
		else
		...
		if ($val == NULL)
			...
			$keyFilter = str_replace("@SystemType_Idn@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "SystemTypesedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "SystemTypesadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("SystemTypesview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("SystemTypesview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "SystemTypesadd.php?" . $this->getUrlParm($parm);
		else
			$url = "SystemTypesadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("SystemTypesedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("SystemTypesedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("SystemTypesadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("SystemTypesadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("SystemTypesdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "SystemType_Idn:" . JsonEncode($this->SystemType_Idn->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($this->SystemType_Idn->CurrentValue != NULL) {
			$url .= "SystemType_Idn=" . urlencode($this->SystemType_Idn->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("SystemType_Idn") !== NULL)
				$arKeys[] = Param("SystemType_Idn");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->SystemType_Idn->CurrentValue = $key;
			else
				$this->SystemType_Idn->OldValue = $key;
