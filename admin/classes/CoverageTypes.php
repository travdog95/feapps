...
/**
 * Table class for CoverageTypes
 */
class CoverageTypes extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'CoverageTypes';
		$this->TableName = 'CoverageTypes';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "[dbo].[CoverageTypes]";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// CoverageType_Idn
		$this->CoverageType_Idn = new DbField('CoverageTypes', 'CoverageTypes', 'x_CoverageType_Idn', 'CoverageType_Idn', '[CoverageType_Idn]', 'CAST([CoverageType_Idn] AS NVARCHAR)', 3, 4, -1, FALSE, '[CoverageType_Idn]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->CoverageType_Idn->IsAutoIncrement = TRUE; // Autoincrement field
		$this->CoverageType_Idn->IsPrimaryKey = TRUE; // Primary key field
		$this->CoverageType_Idn->Nullable = FALSE; // NOT NULL field
		...
		// Name
		$this->Name = new DbField('CoverageTypes', 'CoverageTypes', 'x_Name', 'Name', '[Name]', '[Name]', 200, 100, -1, FALSE, '[Name]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Name->Sortable = TRUE; // Allow sort
		$this->fields['Name'] = &$this->Name;

		// ShortName
		$this->ShortName = new DbField('CoverageTypes', 'CoverageTypes', 'x_ShortName', 'ShortName', '[ShortName]', '[ShortName]', 200, 30, -1, FALSE, '[ShortName]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ShortName->Sortable = TRUE; // Allow sort
		$this->fields['ShortName'] = &$this->ShortName;

		// Rank
		$this->Rank = new DbField('CoverageTypes', 'CoverageTypes', 'x_Rank', 'Rank', '[Rank]', 'CAST([Rank] AS NVARCHAR)', 3, 4, -1, FALSE, '[Rank]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ActiveFlag
		$this->ActiveFlag = new DbField('CoverageTypes', 'CoverageTypes', 'x_ActiveFlag', 'ActiveFlag', '[ActiveFlag]', '[ActiveFlag]', 11, 2, -1, FALSE, '[ActiveFlag]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		...
		$this->ActiveFlag->Lookup = new Lookup('ActiveFlag', 'CoverageTypes', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[CoverageTypes]";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->CoverageType_Idn->setDbValue($conn->insert_ID());
			$rs['CoverageType_Idn'] = $this->CoverageType_Idn->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('CoverageType_Idn', $rs))
				AddFilter($where, QuotedName('CoverageType_Idn', $this->Dbid) . '=' . QuotedValue($rs['CoverageType_Idn'], $this->CoverageType_Idn->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	{
		return "[CoverageType_Idn] = @CoverageType_Idn@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('CoverageType_Idn', $row) ? $row['CoverageType_Idn'] : NULL;
		else
		...
		if ($val == NULL)
			...
			$keyFilter = str_replace("@CoverageType_Idn@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	{
		global $Language;
		if ($pageName == "CoverageTypesview.php")
			return $Language->phrase("View");
		elseif ($pageName == "CoverageTypesedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "CoverageTypesadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("CoverageTypesview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("CoverageTypesview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "CoverageTypesadd.php?" . $this->getUrlParm($parm);
		else
			$url = "CoverageTypesadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("CoverageTypesedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("CoverageTypesadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("CoverageTypesdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "CoverageType_Idn:" . JsonEncode($this->CoverageType_Idn->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->CoverageType_Idn->CurrentValue != NULL) {
			$url .= "CoverageType_Idn=" . urlencode($this->CoverageType_Idn->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("CoverageType_Idn") !== NULL)
				$arKeys[] = Param("CoverageType_Idn");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->CoverageType_Idn->CurrentValue = $key;
			else
				$this->CoverageType_Idn->OldValue = $key;
