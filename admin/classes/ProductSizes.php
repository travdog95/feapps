...
/**
 * Table class for ProductSizes
 */
class ProductSizes extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'ProductSizes';
		$this->TableName = 'ProductSizes';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "[dbo].[ProductSizes]";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// ProductSize_Idn
		$this->ProductSize_Idn = new DbField('ProductSizes', 'ProductSizes', 'x_ProductSize_Idn', 'ProductSize_Idn', '[ProductSize_Idn]', 'CAST([ProductSize_Idn] AS NVARCHAR)', 3, 4, -1, FALSE, '[ProductSize_Idn]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ProductSize_Idn->IsAutoIncrement = TRUE; // Autoincrement field
		$this->ProductSize_Idn->IsPrimaryKey = TRUE; // Primary key field
		$this->ProductSize_Idn->Nullable = FALSE; // NOT NULL field
		...
		// Name
		$this->Name = new DbField('ProductSizes', 'ProductSizes', 'x_Name', 'Name', '[Name]', '[Name]', 200, 100, -1, FALSE, '[Name]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Name->Sortable = TRUE; // Allow sort
		$this->fields['Name'] = &$this->Name;

		// Value
		$this->Value = new DbField('ProductSizes', 'ProductSizes', 'x_Value', 'Value', '[Value]', 'CAST([Value] AS NVARCHAR)', 131, 8, -1, FALSE, '[Value]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Rank
		$this->Rank = new DbField('ProductSizes', 'ProductSizes', 'x_Rank', 'Rank', '[Rank]', 'CAST([Rank] AS NVARCHAR)', 3, 4, -1, FALSE, '[Rank]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Department_Idn
		$this->Department_Idn = new DbField('ProductSizes', 'ProductSizes', 'x_Department_Idn', 'Department_Idn', '[Department_Idn]', 'CAST([Department_Idn] AS NVARCHAR)', 3, 4, -1, FALSE, '[Department_Idn]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->Department_Idn->Lookup = new Lookup('Department_Idn', 'jpr_Department', FALSE, 'Description', ["Description","","",""], [], [], [], [], [], [], '[Description] ASC', '');
		$this->Department_Idn->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['Department_Idn'] = &$this->Department_Idn;

		// ActiveFlag
		$this->ActiveFlag = new DbField('ProductSizes', 'ProductSizes', 'x_ActiveFlag', 'ActiveFlag', '[ActiveFlag]', '[ActiveFlag]', 11, 2, -1, FALSE, '[ActiveFlag]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->ActiveFlag->Sortable = TRUE; // Allow sort
		$this->ActiveFlag->DataType = DATATYPE_BOOLEAN;
		$this->ActiveFlag->Lookup = new Lookup('ActiveFlag', 'ProductSizes', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[ProductSizes]";
	}
	...
	{
		return ($this->SqlOrderBy != "") ? $this->SqlOrderBy : "[Value] ASC";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->ProductSize_Idn->setDbValue($conn->insert_ID());
			$rs['ProductSize_Idn'] = $this->ProductSize_Idn->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('ProductSize_Idn', $rs))
				AddFilter($where, QuotedName('ProductSize_Idn', $this->Dbid) . '=' . QuotedValue($rs['ProductSize_Idn'], $this->ProductSize_Idn->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	{
		return "[ProductSize_Idn] = @ProductSize_Idn@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('ProductSize_Idn', $row) ? $row['ProductSize_Idn'] : NULL;
		else
		...
		if ($val == NULL)
			...
			$keyFilter = str_replace("@ProductSize_Idn@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	{
		global $Language;
		if ($pageName == "ProductSizesview.php")
			return $Language->phrase("View");
		elseif ($pageName == "ProductSizesedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "ProductSizesadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("ProductSizesview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("ProductSizesview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "ProductSizesadd.php?" . $this->getUrlParm($parm);
		else
			$url = "ProductSizesadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("ProductSizesedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("ProductSizesadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("ProductSizesdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "ProductSize_Idn:" . JsonEncode($this->ProductSize_Idn->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->ProductSize_Idn->CurrentValue != NULL) {
			$url .= "ProductSize_Idn=" . urlencode($this->ProductSize_Idn->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("ProductSize_Idn") !== NULL)
				$arKeys[] = Param("ProductSize_Idn");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->ProductSize_Idn->CurrentValue = $key;
			else
				$this->ProductSize_Idn->OldValue = $key;
