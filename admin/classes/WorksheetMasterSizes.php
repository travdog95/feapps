...
/**
 * Table class for WorksheetMasterSizes
 */
class WorksheetMasterSizes extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'WorksheetMasterSizes';
		$this->TableName = 'WorksheetMasterSizes';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "[dbo].[WorksheetMasterSizes]";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// WorksheetMaster_Idn
		$this->WorksheetMaster_Idn = new DbField('WorksheetMasterSizes', 'WorksheetMasterSizes', 'x_WorksheetMaster_Idn', 'WorksheetMaster_Idn', '[WorksheetMaster_Idn]', 'CAST([WorksheetMaster_Idn] AS NVARCHAR)', 3, 4, -1, FALSE, '[WorksheetMaster_Idn]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->WorksheetMaster_Idn->Lookup = new Lookup('WorksheetMaster_Idn', 'WorksheetMasters', FALSE, 'WorksheetMaster_Idn', ["Name","","",""], [], [], [], [], [], [], '[Rank] ASC', '');
		...
		// ProductSize_Idn
		$this->ProductSize_Idn = new DbField('WorksheetMasterSizes', 'WorksheetMasterSizes', 'x_ProductSize_Idn', 'ProductSize_Idn', '[ProductSize_Idn]', 'CAST([ProductSize_Idn] AS NVARCHAR)', 3, 4, -1, FALSE, '[ProductSize_Idn]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->ProductSize_Idn->IsPrimaryKey = TRUE; // Primary key field
		$this->ProductSize_Idn->Nullable = FALSE; // NOT NULL field
		$this->ProductSize_Idn->Required = TRUE; // Required field
		...
		$this->ProductSize_Idn->Lookup = new Lookup('ProductSize_Idn', 'ProductSizes', FALSE, 'ProductSize_Idn', ["Name","","",""], [], [], [], [], [], [], '[Name] ASC', '');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[WorksheetMasterSizes]";
	}
	...
	{
		return ($this->SqlOrderBy != "") ? $this->SqlOrderBy : "[ProductSize_Idn] ASC";
	}
	...
	{
		return "[WorksheetMaster_Idn] = @WorksheetMaster_Idn@ AND [ProductSize_Idn] = @ProductSize_Idn@";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "WorksheetMasterSizeslist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "WorksheetMasterSizesview.php")
			return $Language->phrase("View");
		elseif ($pageName == "WorksheetMasterSizesedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "WorksheetMasterSizesadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("WorksheetMasterSizesview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("WorksheetMasterSizesview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "WorksheetMasterSizesadd.php?" . $this->getUrlParm($parm);
		else
			$url = "WorksheetMasterSizesadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("WorksheetMasterSizesedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("WorksheetMasterSizesadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("WorksheetMasterSizesdelete.php", $this->getUrlParm());
	}
		...
		if (is_array($arKeys)) {
			foreach ($arKeys as $key) {
				...
				if (!is_numeric($key[0])) // WorksheetMaster_Idn
					continue;
				if (!is_numeric($key[1])) // ProductSize_Idn
					continue;
				$ar[] = $key;
			}
		}
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->ProductSize_Idn->CurrentValue = $key[1];
			else
				$this->ProductSize_Idn->OldValue = $key[1];
