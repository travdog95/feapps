...
/**
 * Table class for HeadTypes
 */
class HeadTypes extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'HeadTypes';
		$this->TableName = 'HeadTypes';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "[dbo].[HeadTypes]";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// HeadType_Idn
		$this->HeadType_Idn = new DbField('HeadTypes', 'HeadTypes', 'x_HeadType_Idn', 'HeadType_Idn', '[HeadType_Idn]', 'CAST([HeadType_Idn] AS NVARCHAR)', 3, 4, -1, FALSE, '[HeadType_Idn]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->HeadType_Idn->IsAutoIncrement = TRUE; // Autoincrement field
		$this->HeadType_Idn->IsPrimaryKey = TRUE; // Primary key field
		$this->HeadType_Idn->Nullable = FALSE; // NOT NULL field
		...
		// Name
		$this->Name = new DbField('HeadTypes', 'HeadTypes', 'x_Name', 'Name', '[Name]', '[Name]', 200, 100, -1, FALSE, '[Name]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Name->Sortable = TRUE; // Allow sort
		$this->fields['Name'] = &$this->Name;

		// ShortName
		$this->ShortName = new DbField('HeadTypes', 'HeadTypes', 'x_ShortName', 'ShortName', '[ShortName]', '[ShortName]', 200, 30, -1, FALSE, '[ShortName]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ShortName->Sortable = TRUE; // Allow sort
		$this->fields['ShortName'] = &$this->ShortName;

		// Rank
		$this->Rank = new DbField('HeadTypes', 'HeadTypes', 'x_Rank', 'Rank', '[Rank]', 'CAST([Rank] AS NVARCHAR)', 3, 4, -1, FALSE, '[Rank]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// ActiveFlag
		$this->ActiveFlag = new DbField('HeadTypes', 'HeadTypes', 'x_ActiveFlag', 'ActiveFlag', '[ActiveFlag]', '[ActiveFlag]', 11, 2, -1, FALSE, '[ActiveFlag]', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->ActiveFlag->Sortable = TRUE; // Allow sort
		$this->ActiveFlag->DataType = DATATYPE_BOOLEAN;
		$this->ActiveFlag->Lookup = new Lookup('ActiveFlag', 'HeadTypes', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "[dbo].[HeadTypes]";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->HeadType_Idn->setDbValue($conn->insert_ID());
			$rs['HeadType_Idn'] = $this->HeadType_Idn->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('HeadType_Idn', $rs))
				AddFilter($where, QuotedName('HeadType_Idn', $this->Dbid) . '=' . QuotedValue($rs['HeadType_Idn'], $this->HeadType_Idn->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	{
		return "[HeadType_Idn] = @HeadType_Idn@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('HeadType_Idn', $row) ? $row['HeadType_Idn'] : NULL;
		else
		...
		if ($val == NULL)
			...
			$keyFilter = str_replace("@HeadType_Idn@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	{
		global $Language;
		if ($pageName == "HeadTypesview.php")
			return $Language->phrase("View");
		elseif ($pageName == "HeadTypesedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "HeadTypesadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("HeadTypesview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("HeadTypesview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "HeadTypesadd.php?" . $this->getUrlParm($parm);
		else
			$url = "HeadTypesadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("HeadTypesedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("HeadTypesadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("HeadTypesdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "HeadType_Idn:" . JsonEncode($this->HeadType_Idn->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->HeadType_Idn->CurrentValue != NULL) {
			$url .= "HeadType_Idn=" . urlencode($this->HeadType_Idn->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("HeadType_Idn") !== NULL)
				$arKeys[] = Param("HeadType_Idn");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->HeadType_Idn->CurrentValue = $key;
			else
				$this->HeadType_Idn->OldValue = $key;
