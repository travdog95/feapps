...
class ADODB_sqlite3 extends ADOConnection {
	 ...
	 * @return integer
	 */
	function SelectRowId($table, $where)
	{
		$sql = "SELECT ROWID FROM $table WHERE $where";
		$rs = parent::Execute($sql, false);
		if (!$rs->EOF)
			$rowId = $rs->fields(0);
		else
			$rowId = 0;
	 ...
	 * Return blob data //***
	 *
	 * @param string $table (Table name, NOT quoted)
	 * @param string $column (Column name, NOT quoted)
	 * @param string $id (Row id)
	 * @return string (binary data)
	 */
	function OpenBlob($table, $column, $id)
	{
		$stream = $this->_connectionID->OpenBlob($table, $column, $id);
	 ...
	 *
	 * @param string $table (Table name, quoted)
	 * @param string $column (Column name, quoted)
	 * @param string $val (Binary data)
	 * @param string $where (WHERE clause)
	 * @return boolean
	 */
	function UpdateBlob($table, $column, $val, $where)
	{
		$sql = "UPDATE $table set $column=:blob WHERE $where";
		$stmt = $this->_connectionID->prepare($sql);
		$stmt->bindValue(":blob", $val, SQLITE3_BLOB);
		return $stmt->execute();
	}
	...
	function Execute($sql, $inputarr = false) {
		...
		if ($ret && $ret instanceof ADORecordset_sqlite3 && !$GLOBALS["ADODB_COUNTRECS"] &&
