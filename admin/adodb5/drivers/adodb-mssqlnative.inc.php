...
class ADODB_mssqlnative extends ADOConnection {
	...
	*/
	...
	function _connect($argHostname, $argUsername, $argPassword, $argDatabasename)
	{
		...
		if ($argUsername && $argPassword) { //*** No username and password => trusted connection
	...
	*/
	...
	{
		// Get version from 'SELECT @@version', e.g.
		// Microsoft SQL Server 2016 ... 13.0.4259.0 ... or Microsoft SQL Azure ... 12.0.2000.8 ...
		$_hasOrderBy = @$inputarr["_hasOrderBy"];
		unset($inputarr["_hasOrderBy"]);
		if (!$this->mssql_version)
			$this->ServerVersion();
		if (intval($this->mssql_version) >= 11 && is_bool($_hasOrderBy)) { // Microsoft SQL Server >= 2012 and SQL with ORDER BY clause
