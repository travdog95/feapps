...
class Recap_row_new
{
	...
	public function __construct($params = array('recap_row_idn' => 0, 'job' => array(), 'labor_hours' => array()))
	{
        ...
        else
        {
            ...
            {
                case 1: //Underground
                    $this->_load_underground($job);
                    break;
                case 22: //Shop Labor
                case 29:
                    //Get shop hours
                    $this->recap_cells[5] = ceil($this->shop_hours * $job['job']['shop_labor_rate']);
    ...
    private function _load_underground($job)
    {
        //Delcare and initialize variables
        $worksheet_idn = 0;
        $w;
        ...
        foreach ($this->recap_row_worksheet_masters as $recap_row_worksheet_master)
        {
            ...
            if ($recap_row_worksheet_master['WorksheetMaster_Idn'] == 11)
            {
                ...
                if ($worksheet_idn > 0)
                {
                    ...
                    $this->recap_cells[5] = ceil(ceil($w->field_hours) * $job['job_parms'][65]['NumericValue']);
    ...
    function _load_row($job)
    {
        ...
        if ($worksheet_idn > 0)
        {
            if ($this->recap_row_idn == 8 || $this->recap_row_idn == 32) //Job Mob
            {
                ...
                $this->CI->load->library('job_mob', array('w_id' => $worksheet_idn, 'j' => null));
                $w = $this->CI->job_mob;
                ...
                if ($recap_cell !== false)
                {
                    switch($column)
                    {
                        ...
                        case 5: //Labor
                            ...
                            {
                                $this->recap_cells[$column] = ceil(ceil($w->engineering_hours) * $job['job']['design_labor_rate']);
                            }
                            ...
                            {
                                $this->recap_cells[$column] = ceil(ceil($w->field_hours) * $job['job']['field_labor_rate']);
                            }
                            break;
                        case 6: //Hours
                            if ($this->recap_row_idn == 7  || $this->recap_row_idn == 31) //Engineering
