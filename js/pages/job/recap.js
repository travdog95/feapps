...
$(function () {
	...
	$("#generatePDF").on("click", function (e) {
		...
		$("#generatePDF").hide();
		//$("#genmsg").show();
		html2canvas($("#pdfContent").get(0), { allowTaint: true }).then(function (
			canvas
		) {
			...
			pdf.addImage(imgData, "JPG", 0.25, 0.25, 8, 10.5);
...
function recap_handlers() {
	...
	$("#view_child_jobs").click(function (e) {
		...
		$.get(
			FECI.base_url + "job/get_child_jobs/" + FECI.job.job_number,
			function (html) {
	...
	$("#lock_job").on("click", function (e) {
		e.preventDefault();
		//Load dialog body
		$("#ConfirmationModal .modal-title").html("Confirm Lock Job");
		$("#ConfirmationModal .modal-body p").html(`Lock job ${FECI.job.name}?`);
	...
	$("#unlock_job").on("click", function (e) {
		e.preventDefault();
		//Load dialog body
		$("#ConfirmationModal .modal-title").html("Confirm Unlock Job");
		$("#ConfirmationModal .modal-body p").html(`Unlock job ${FECI.job.name}?`);
	...
	$("#Confirmed").on("click", function (e) {
		//Lock job
		if (FECI.confirmation_modal.action === "lock-job") {
			lock_job(1, FECI.confirmation_modal.data.jobNumber);
		}

		//Unlock job
		if (FECI.confirmation_modal.action === "unlock-job") {
			lock_job(0, FECI.confirmation_modal.data.jobNumber);
		}
	});
}
...
function lock_job(is_locked, job_number) {
	//Disable OK button
	...
	//AJAX request
	FECI.request = $.ajax({
		url: FECI.base_url + "job/lock_job",
		type: "POST",
		data: {
			is_locked: is_locked,
	...
	FECI.request.done(function (response, textStatus, jqXHR) {
		...
		const message =
			is_locked === 1 ? `${FECI.job.name} locked` : `${FECI.job.name} unlocked`;

		if (res.return_code == 1) {
			//refresh page
			location.href = `${FECI.base_url}job/recap/${FECI.job.job_number}?message=${message}&message_type=1`;
		}
	});

	//Failure callback handler
	FECI.request.fail(function (jqXHR, textStatus, errorThrown) {
		displayMessageBox("Error locking/unlocking job", "danger");
	});
...
function user_changed_total() {
	...
	if (user_total < case2_total) {
		...
		if (user_total >= case1_total) {
			...
			capacity_cost_allocation =
				case1_diff * parseFloat(FECI.job.job_parms[55]["NumericValue"]);
			commission_allocation =
				case1_diff * parseFloat(FECI.job.job_parms[56]["NumericValue"]);
			...
			case2_total_direct_cost =
				high_sub_total_direct_cost + labor_total_direct_cost;

			high_sub_percentage =
				high_sub_total_direct_cost > 0
					? parseFloat(
							precise_round(
								high_sub_total_direct_cost / case2_total_direct_cost,
								6
							)
					  )
					: 0;
			labor_percentage =
				case2_total_direct_cost > 0
					? parseFloat(
							precise_round(
								labor_total_direct_cost / case2_total_direct_cost,
								6
							)
					  )
					: 0;
			...
			high_sub_subtotal_mark_up =
				high_sub_total_direct_cost * FECI.job.job_parms[52]["NumericValue"];
			labor_subtotal_mark_up =
				labor_total_direct_cost * FECI.job.job_parms[53]["NumericValue"];
			...
			high_sub_total_after_mark_up =
				high_sub_mark_up + high_sub_total_direct_cost;
			labor_total_after_mark_up = labor_mark_up + labor_total_direct_cost;

			var high_sub_mark_up_percent = 0.0;
			var labor_mark_up_percent = 0.0;

			high_sub_mark_up_percent =
				high_sub_total_direct_cost > 0
					? parseFloat(
							precise_round(high_sub_mark_up / high_sub_total_direct_cost, 6)
					  )
					: 0;
			labor_mark_up_percent =
				labor_total_direct_cost > 0
					? parseFloat(
							precise_round(labor_mark_up / labor_total_direct_cost, 6)
					  )
					: 0;

			//Load new percentages into fields
			$("#mark_up_percent_3").val(
				parseFloat(precise_round(high_sub_mark_up_percent, 6) * 100)
			);
			$("#mark_up_percent_5").val(
				parseFloat(precise_round(labor_mark_up_percent, 6) * 100)
			);
			...
			$(".total_capacity_cost").each(function (index) {
				if (index !== 2 && index !== 4) {
					total_static_after_capacity_costs += parseInt(
						strip_comma($(this).html())
					);
				}
			});

			//Add calculated amounts for columns 3 & 5
			total_after_capacity_costs =
				total_static_after_capacity_costs +
				high_sub_total_after_mark_up +
				labor_total_after_mark_up;

			var total_after_mark_up =
				total_static_after_capacity_costs +
				high_sub_total_direct_cost +
				high_sub_subtotal_mark_up +
				labor_total_direct_cost +
				labor_subtotal_mark_up;
			var case1_commission = Math.round(
				total_after_mark_up * FECI.job.job_parms[54]["NumericValue"]
			);

			var commission = case1_commission + commission_allocation;
			var commission_percent = precise_round(
				commission / total_after_capacity_costs,
				6
			);

			$("#regular_commission_percent").val(
				precise_round(parseFloat(commission_percent) * 100, 4)
			);
		...
		if (FECI.job.totals.gross_receipt_percent > 0) {
			total_after_cost_of_bond = Math.round(
				user_total / (1 + FECI.job.totals.gross_receipt_percent)
			);
			gross_receipt = Math.round(
				total_after_cost_of_bond * FECI.job.totals.gross_receipt_percent
			);
		...
		if (FECI.job.totals.cost_of_bond_percent > 0) {
			...
			total_after_depository_fee = Math.round(
				total_after_cost_of_bond / (1 + FECI.job.totals.cost_of_bond_percent)
			);
			cost_of_bond = Math.floor(
				total_after_depository_fee * FECI.job.totals.cost_of_bond_percent
			);
		} else {
			total_after_depository_fee =
				total_after_cost_of_bond - FECI.job.totals.cost_of_bond;
		}
		...
		if (FECI.job.totals.depository_fee_percent > 0) {
			//Calculate Total After Bond
			total_after_sales_tax = Math.round(
				total_after_depository_fee /
					(1 + FECI.job.totals.depository_fee_percent)
			);
			depository_fee = Math.round(
				total_after_sales_tax * FECI.job.totals.depository_fee_percent
			);
		} else {
			total_after_sales_tax =
				total_after_depository_fee - FECI.job.totals.depository_fee;
		}

		//Calculate Total After Profit Mark-Up
		var total_after_profit_mark_up =
			total_after_sales_tax - FECI.job.totals.sales_tax;

		//Calculate Total Profit Mark-Up
		FECI.job.totals.profit_mark_up =
			total_after_profit_mark_up - Math.round(FECI.job.totals.subtotal);
		$("#profit_mark_up").val(
			number_format(FECI.job.totals.profit_mark_up, 0, ",")
		);
...
function calc_recap() {
	...
	//Total
	FECI.job.totals.total =
		FECI.job.totals.gross_receipt + FECI.job.totals.total_after_cost_of_bond;
	$("#total").val(number_format(FECI.job.totals.total, 0, ","));

	//Case 1
	FECI.job.case1_totals.total =
		FECI.job.case1_totals.gross_receipt +
		FECI.job.case1_totals.total_after_cost_of_bond;
	$("#case1_total").text(number_format(FECI.job.case1_totals.total, 0, ","));

	//Case 2
	FECI.job.case2_totals.total =
		FECI.job.case2_totals.gross_receipt +
		FECI.job.case2_totals.total_after_cost_of_bond;
...
function calc_pac() {
	//Calculate PAC
	FECI.job.subtotals.pac = Math.round(
		parseInt(strip_comma($("#item_total_5").text())) *
			FECI.job.payroll_added_costs
	);
	$("#pac_5").text(number_format(FECI.job.subtotals.pac, 0, ","));

	//Total after PAC
	FECI.job.subtotals.total_after_pac =
		FECI.job.subtotals.pac + FECI.job.subtotals.item_totals[5];
	$("#total_after_pac_5").text(
		number_format(FECI.job.subtotals.total_after_pac, 0, ",")
	);
}

function calc_supervisory_fee() {
	//Supervisory fee
	FECI.job.subtotals.supervisory_fee_percent =
		$("#supervisory_fee_percent").val() == ""
			? 0
			: parseFloat(number_format($("#supervisory_fee_percent").val(), 1, "")) /
			  100;
	FECI.job.subtotals.supervisory_fee =
		FECI.job.subtotals.supervisory_fee_percent *
		FECI.job.subtotals.total_after_pac;

	$("#supervisory_fee").text(
		number_format(FECI.job.subtotals.supervisory_fee, 0, ",")
	);
	$("#supervisory_fee_percent").val(
		number_format(FECI.job.subtotals.supervisory_fee_percent * 100, 1, ",")
	);
}
...
function calc_contingencies() {
	...
	for (i = 2; i <= 5; i++) {
		...
		if ($("#override_contingency_" + i + "_percent").val() == 1) {
			...
			FECI.job.subtotals.contingencies[i] = parseFloat(
				strip_comma($("#contingency_" + i).val())
			);

			//Apply inactive class to contingency
			$("#contingency_" + i + "_percent").addClass("inactive");
		} else {
			FECI.job.subtotals.contingency_percents[i] =
				parseFloat(
					number_format($("#contingency_" + i + "_percent").val(), 1, "")
				) / 100;
			//If labor, calculate from Total after PAC
			amount =
				i == 5
					? FECI.job.subtotals.total_after_pac
					: FECI.job.subtotals.item_totals[i];

			FECI.job.subtotals.contingencies[i] = Math.round(
				FECI.job.subtotals.contingency_percents[i] * amount
			);
		...
		$("#contingency_" + i).val(
			number_format(FECI.job.subtotals.contingencies[i], 0, ",")
		);
		$("#contingency_" + i + "_percent").val(
			number_format(FECI.job.subtotals.contingency_percents[i] * 100, 1, ",")
		);
	}
}
...
function calc_total_direct_costs() {
	...
	for (i = 1; i <= 5; i++) {
		//Set to Item Total
		FECI.job.subtotals.total_direct_costs[i] =
			FECI.job.subtotals.item_totals[i];
		if (i > 1) {
			//If Low Sub, High Sub, Material or Labor add contingencies
			FECI.job.subtotals.total_direct_costs[i] +=
				FECI.job.subtotals.contingencies[i];

			//If Labor, add PAC and Supervisory fee
			if (i === 5) {
				FECI.job.subtotals.total_direct_costs[i] +=
					FECI.job.subtotals.pac + FECI.job.subtotals.supervisory_fee;
			}
		}

		grandTotal += parseInt(
			number_format(FECI.job.subtotals.total_direct_costs[i], 0, "")
		);

		//Set field value
		$("#total_direct_cost_" + i).text(
			number_format(FECI.job.subtotals.total_direct_costs[i], 0, ",")
		);
	}
...
function calc_mark_up_costs() {
	...
	for (i = 1; i <= 5; i++) {
		...
		if (i === 1) {
			...
			mark_up_percent = $("#mark_up_percent_" + i).is("input")
				? $("#mark_up_percent_" + i).val()
				: $("#mark_up_percent_" + i).text();

			FECI.job.subtotals.mark_up_percents[i] =
				mark_up_percent == ""
					? 0
					: parseFloat(number_format(mark_up_percent, 6, "")) / 100;

			FECI.job.subtotals.mark_ups[i] = Math.round(
				FECI.job.subtotals.mark_up_percents[i] *
					FECI.job.subtotals.total_direct_costs[i]
			);
		}
		...
		$("#mark_up_" + i).text(
			number_format(FECI.job.subtotals.mark_ups[i], 0, ",")
		);

		//Format percents for columns 3 (high sub) and 5 (labor)
		if (i == 3 || i == 5) {
			$("#mark_up_percent_" + i).val(
				number_format(FECI.job.subtotals.mark_up_percents[i] * 100, 4, ",")
			);
		}

		markUpCostsTotal += parseInt(
			number_format(FECI.job.subtotals.mark_ups[i], 0, "")
		);
	}
	...
	//Case 1
	FECI.job.case1_subtotals.mark_ups[2] = Math.round(
		FECI.job.job_parms[38]["NumericValue"] *
			FECI.job.subtotals.total_direct_costs[2]
	);
	FECI.job.case1_subtotals.mark_ups[3] = Math.round(
		FECI.job.job_parms[52]["NumericValue"] *
			FECI.job.subtotals.total_direct_costs[3]
	);
	FECI.job.case1_subtotals.mark_ups[4] = Math.round(
		FECI.job.job_parms[40]["NumericValue"] *
			FECI.job.subtotals.total_direct_costs[4]
	);
	FECI.job.case1_subtotals.mark_ups[5] = Math.round(
		FECI.job.job_parms[53]["NumericValue"] *
			FECI.job.subtotals.total_direct_costs[5]
	);

	//Case 2
	FECI.job.case2_subtotals.mark_ups[2] = Math.round(
		FECI.job.job_parms[38]["NumericValue"] *
			FECI.job.subtotals.total_direct_costs[2]
	);
	FECI.job.case2_subtotals.mark_ups[3] = Math.round(
		FECI.job.job_parms[39]["NumericValue"] *
			FECI.job.subtotals.total_direct_costs[3]
	);
	FECI.job.case2_subtotals.mark_ups[4] = Math.round(
		FECI.job.job_parms[40]["NumericValue"] *
			FECI.job.subtotals.total_direct_costs[4]
	);
	FECI.job.case2_subtotals.mark_ups[5] = Math.round(
		FECI.job.job_parms[41]["NumericValue"] *
			FECI.job.subtotals.total_direct_costs[5]
	);
}
...
function calc_total_capacity_costs() {
	...
	for (i = 1; i <= 5; i++) {
		FECI.job.subtotals.total_capacity_costs[i] =
			FECI.job.subtotals.total_direct_costs[i] + FECI.job.subtotals.mark_ups[i];

		//Add to total of Capacity Costs
		total_capacity_cost += FECI.job.subtotals.total_capacity_costs[i];

		//Format and load into UI
		$("#total_capacity_cost_" + i).text(
			number_format(FECI.job.subtotals.total_capacity_costs[i], 0, ",")
		);

		//Case 1
		FECI.job.case1_subtotals.total_capacity_costs[i] =
			FECI.job.subtotals.total_direct_costs[i] +
			FECI.job.case1_subtotals.mark_ups[i];
		case1_total_capacity_cost +=
			FECI.job.case1_subtotals.total_capacity_costs[i];

		//Case 2
		FECI.job.case2_subtotals.total_capacity_costs[i] =
			FECI.job.subtotals.total_direct_costs[i] +
			FECI.job.case2_subtotals.mark_ups[i];
		case2_total_capacity_cost +=
			FECI.job.case2_subtotals.total_capacity_costs[i];
	}
	...
	$("#total_capacity_cost").text(
		number_format(FECI.job.totals.total_capacity_cost, 0, ",")
	);
}
...
function calc_commission() {
	...
	//Regular Commission
	FECI.job.totals.regular_commission_percent =
		$("#regular_commission_percent").val() == ""
			? 0
			: parseFloat(
					number_format($("#regular_commission_percent").val(), 4, "")
			  ) / 100;
	$("#regular_commission_percent").val(
		number_format(FECI.job.totals.regular_commission_percent * 100, 4, ",")
	);

	FECI.job.totals.regular_commission = Math.round(
		FECI.job.totals.total_capacity_cost *
			FECI.job.totals.regular_commission_percent
	);
	$("#regular_commission").text(
		number_format(FECI.job.totals.regular_commission, 0, ",")
	);

	//Case 1
	FECI.job.case1_totals.regular_commission = Math.round(
		FECI.job.case1_totals.total_capacity_cost *
			parseFloat(FECI.job.job_parms[54]["NumericValue"])
	);

	//Case 2
	case2_commission_percent =
		FECI.job.department_idn == 1
			? FECI.job.job_parms[13]["NumericValue"]
			: FECI.job.job_parms[70]["NumericValue"];
	FECI.job.case2_totals.regular_commission = Math.round(
		FECI.job.case2_totals.total_capacity_cost *
			parseFloat(case2_commission_percent)
	);

	//Subtotal
	FECI.job.totals.subtotal =
		FECI.job.totals.total_capacity_cost + FECI.job.totals.regular_commission;
	$("#subtotal").text(number_format(FECI.job.totals.subtotal, 0, ","));

	//Case 1
	FECI.job.case1_totals.subtotal =
		FECI.job.case1_totals.total_capacity_cost +
		FECI.job.case1_totals.regular_commission;

	//Case 2
	FECI.job.case2_totals.subtotal =
		FECI.job.case2_totals.total_capacity_cost +
		FECI.job.case2_totals.regular_commission;
}
...
function calc_profit_mark_up() {
	...
	var override_profit_mark_up_percent = $(
		"#override_profit_mark_up_percent"
	).val();

	//If user manually inputs Profit Mark Up dollar amount
	if (override_profit_mark_up_percent == 1) {
		FECI.job.totals.profit_mark_up_percent = 0;
		FECI.job.totals.profit_mark_up = parseFloat(
			strip_comma($("#profit_mark_up").val())
		);

		//calculate approximate percentage for display purposes only
		$(".approx-profit-mark-up-percent").removeClass("display_none");
		$("#ApproximateProfitMarkUpPercent").text(
			number_format(
				(FECI.job.totals.profit_mark_up / FECI.job.totals.subtotal) * 100,
				2,
				","
			)
		);
		...
		FECI.job.totals.profit_mark_up_percent =
			$("#profit_mark_up_percent").val() == ""
				? 0
				: parseFloat(number_format($("#profit_mark_up_percent").val(), 2, "")) /
				  100;
		FECI.job.totals.profit_mark_up = Math.round(
			FECI.job.totals.profit_mark_up_percent * FECI.job.totals.subtotal
		);
	...
	$("#profit_mark_up_percent").val(
		number_format(FECI.job.totals.profit_mark_up_percent * 100, 2, ",")
	);
	$("#profit_mark_up").val(
		number_format(FECI.job.totals.profit_mark_up, 0, ",")
	);
	...
	FECI.job.totals.total_after_profit_mark_up =
		FECI.job.totals.subtotal + FECI.job.totals.profit_mark_up;
	$("#total_after_profit_mark_up").text(
		number_format(FECI.job.totals.total_after_profit_mark_up, 0, ",")
	);

	//Case 1
	FECI.job.case1_totals.profit_mark_up = 0;
	FECI.job.case1_totals.total_after_profit_mark_up =
		FECI.job.case1_totals.subtotal;

	//Case 2
	FECI.job.case2_totals.profit_mark_up = 0;
	FECI.job.case2_totals.total_after_profit_mark_up =
		FECI.job.case2_totals.subtotal;
}

function calc_sales_tax() {
	//Sales Tax
	FECI.job.totals.sales_tax_percent =
		$("#sales_tax_percent").val() == ""
			? 0
			: parseFloat(number_format($("#sales_tax_percent").val(), 2, "")) / 100;
	$("#sales_tax_percent").val(
		number_format(FECI.job.totals.sales_tax_percent * 100, 2, ",")
	);

	//Sales tax is only a percentage of Total Material Direct Cost
	FECI.job.totals.sales_tax = Math.round(
		FECI.job.totals.sales_tax_percent * FECI.job.subtotals.total_direct_costs[4]
	);
	$("#sales_tax").text(number_format(FECI.job.totals.sales_tax, 0, ","));

	//Total after Sales Tax
	FECI.job.totals.total_after_sales_tax =
		FECI.job.totals.sales_tax + FECI.job.totals.total_after_profit_mark_up;
	$("#total_after_sales_tax").text(
		number_format(FECI.job.totals.total_after_sales_tax, 0, ",")
	);

	//Case 1
	FECI.job.case1_totals.sales_tax = FECI.job.totals.sales_tax;
	FECI.job.case1_totals.total_after_sales_tax =
		FECI.job.case1_totals.total_after_profit_mark_up +
		FECI.job.totals.sales_tax;

	//Case 2
	FECI.job.case2_totals.sales_tax = FECI.job.totals.sales_tax;
	FECI.job.case2_totals.total_after_sales_tax =
		FECI.job.case2_totals.total_after_profit_mark_up +
		FECI.job.totals.sales_tax;
}

function calc_depository_fee() {
	FECI.job.totals.depository_fee_percent =
		$("#depository_fee_percent").val() == ""
			? 0
			: parseFloat(number_format($("#depository_fee_percent").val(), 2, "")) /
			  100;
	FECI.job.totals.depository_fee =
		$("#depository_fee").val() == ""
			? 0
			: parseInt(number_format(strip_comma($("#depository_fee").val()), 0, ""));
	FECI.job.totals.override_depository_fee_percent = $(
		"#override_depository_fee_percent"
	).val();
	...
	if (FECI.job.totals.override_depository_fee_percent == 1) {
		...
		FECI.job.totals.depository_fee = Math.round(
			FECI.job.totals.total_after_sales_tax *
				FECI.job.totals.depository_fee_percent
		);
		...
		//Case 1
		FECI.job.case1_totals.depository_fee = Math.round(
			FECI.job.case1_totals.total_after_sales_tax *
				FECI.job.totals.depository_fee_percent
		);

		//Case 2
		FECI.job.case2_totals.depository_fee = Math.round(
			FECI.job.case2_totals.total_after_sales_tax *
				FECI.job.totals.depository_fee_percent
		);
	}
	...
	$("#depository_fee").val(
		number_format(FECI.job.totals.depository_fee, 0, ",")
	);
	$("#depository_fee_percent").val(
		number_format(FECI.job.totals.depository_fee_percent * 100, 2, ",")
	);

	//Total after depository fee
	FECI.job.totals.total_after_depository_fee =
		FECI.job.totals.total_after_sales_tax + FECI.job.totals.depository_fee;
	$("#total_after_depository_fee").text(
		number_format(FECI.job.totals.total_after_depository_fee, 0, ",")
	);

	//Case 1
	FECI.job.case1_totals.total_after_depository_fee =
		FECI.job.case1_totals.total_after_sales_tax +
		FECI.job.case1_totals.depository_fee;

	//Case 2
	FECI.job.case2_totals.total_after_depository_fee =
		FECI.job.case2_totals.total_after_sales_tax +
		FECI.job.case2_totals.depository_fee;
...
function check_depository_fee_limit() {
	...
	if (
		depository_fee > limit &&
		$("#override_depository_fee_limit").is(":checked") === false
	) {
		alert(
			"'Depository Fee' is over the limit and will be changed to $" +
				limit +
				"!"
		);
...
function calc_cost_of_bond() {
	FECI.job.totals.cost_of_bond =
		$("#cost_of_bond").val() == ""
			? 0
			: parseFloat(strip_comma($("#cost_of_bond").val()));
	FECI.job.totals.cost_of_bond_percent =
		$("#cost_of_bond_percent").val() == ""
			? 0
			: parseFloat(number_format($("#cost_of_bond_percent").val(), 2, "")) /
			  100;
	FECI.job.totals.override_cost_of_bond_percent = $(
		"#override_cost_of_bond_percent"
	).val();
	...
	if (FECI.job.totals.override_cost_of_bond_percent == 1) {
		...
		FECI.job.totals.cost_of_bond = Math.round(
			FECI.job.totals.cost_of_bond_percent *
				FECI.job.totals.total_after_depository_fee
		);
		...
		//Case 1
		FECI.job.case1_totals.cost_of_bond = Math.round(
			FECI.job.case1_totals.total_after_depository_fee *
				FECI.job.totals.cost_of_bond_percent
		);

		//Case 2
		FECI.job.case2_totals.cost_of_bond = Math.round(
			FECI.job.case2_totals.total_after_depository_fee *
				FECI.job.totals.cost_of_bond_percent
		);
	}
	...
	$("#cost_of_bond_percent").val(
		number_format(FECI.job.totals.cost_of_bond_percent * 100, 2, ",")
	);

	$("#cost_of_bond").val(number_format(FECI.job.totals.cost_of_bond, 0, ","));

	//Total after cost of bond
	FECI.job.totals.total_after_cost_of_bond =
		FECI.job.totals.cost_of_bond + FECI.job.totals.total_after_depository_fee;

	$("#total_after_cost_of_bond").html(
		number_format(FECI.job.totals.total_after_cost_of_bond, 0, ",")
	);

	//Case 1
	FECI.job.case1_totals.total_after_cost_of_bond =
		FECI.job.case1_totals.total_after_depository_fee +
		FECI.job.case1_totals.cost_of_bond;

	//Case 2
	FECI.job.case2_totals.total_after_cost_of_bond =
		FECI.job.case2_totals.total_after_depository_fee +
		FECI.job.case2_totals.cost_of_bond;
}

function calc_gross_receipt() {
	FECI.job.totals.gross_receipt =
		$("#gross_receipt").val() == ""
			? 0
			: parseFloat(strip_comma($("#gross_receipt").val()));
	FECI.job.totals.gross_receipt_percent =
		$("#gross_receipt_percent").val() == ""
			? 0
			: parseFloat(number_format($("#gross_receipt_percent").val(), 2, "")) /
			  100;
	FECI.job.totals.override_gross_receipt_percent = $(
		"#override_gross_receipt_percent"
	).val();
	...
	if (FECI.job.totals.override_gross_receipt_percent == 1) {
		...
		FECI.job.totals.gross_receipt = Math.round(
			FECI.job.totals.gross_receipt_percent *
				FECI.job.totals.total_after_cost_of_bond
		);
		...
		//Case 1
		FECI.job.case1_totals.gross_receipt = Math.round(
			FECI.job.case1_totals.total_after_cost_of_bond *
				FECI.job.totals.gross_receipt_percent
		);

		//Case 2
		FECI.job.case2_totals.gross_receipt = Math.round(
			FECI.job.case2_totals.total_after_cost_of_bond *
				FECI.job.totals.gross_receipt_percent
		);
	}
	...
	$("#gross_receipt_percent").val(
		number_format(FECI.job.totals.gross_receipt_percent * 100, 2, ",")
	);
...
function calc_sold_subtotal(total_amount) {
	...
	if (isNaN(total_amount) === false && total_amount > 0) {
		...
		if (depository_fee_percentage > 0) {
			sold_subtotal = Math.round(
				sold_subtotal / (1 + depository_fee_percentage)
			);
		} else {
...
function build_parent_element() {
	...
	if (FECI.job.is_parent == 1) {
		element_content =
			'<a href="#" id="view_child_jobs" title="View Child Jobs">Child Jobs</a>';
	} else if (FECI.job.parent_idn > 0) {
		element_content =
			'<a href="/job/recap/' +
			FECI.job.parent_idn +
			'" title="View Parent Recap">View Parent Recap</a>';
	} else {
		element_content =
			'<a href="#" id="add_parent" title="Add Parent">Add Parent Job</a>';
	}
...
function recap_modal_handlers() {
	if ($._data($(".remove_child_jobs")[0], "events") == undefined) {
		...
		$(".remove_child_jobs").click(function (e) {
			...
			if (jobs_checked > 0) {
				//Load dialog body
				$("#remove_child_jobs_modal .modal-body p").html(
...
function calc_budget_summary() {
	...
	let total_sqft = $("#bs_total_sqft").is("input")
		? $("#bs_total_sqft").val()
		: $("#bs_total_sqft").text();
